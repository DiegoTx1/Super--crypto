...
function calcularVolumeProfile(dados, periodo = CONFIG.PERIODOS.VOLUME_PROFILE) {
  try {
    ...
    for (const vela of slice) {
      ...
      const precisao = Math.max(
        vela.high.toString().split('.')[1]?.length || 0,
        vela.low.toString().split('.')[1]?.length || 0,
        2
      );
      
      for (let p = vela.low; p <= vela.high; p += passo) {
        const nivel = p.toFixed(precisao);
        precosUnicos.add(nivel);
        volumePorPreco[nivel] = (volumePorPreco[nivel] || 0) + vela.volume;
      }
    }
    
    const niveis = Array.from(precosUnicos)
      .map(nivel => [parseFloat(nivel), volumePorPreco[nivel]])
      .sort((a, b) => b[1] - a[1]);
    
    if (niveis.length === 0) return { pvp: 0, vaHigh: 0, vaLow: 0 };
    
    const pvp = niveis[0][0];
    const vaHigh = niveis[Math.floor(niveis.length * 0.3)]?.[0] || pvp;
    const vaLow = niveis[Math.floor(niveis.length * 0.7)]?.[0] || pvp;
